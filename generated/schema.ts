...
export class ItemCanceld extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ItemCanceld entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ItemCanceld must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ItemCanceld", id.toString(), this);
    }
  }

  static load(id: string): ItemCanceld | null {
    return changetype<ItemCanceld | null>(store.get("ItemCanceld", id));
  }
